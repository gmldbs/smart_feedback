...
export default {
  ...
  firestore () {
    return {
        data : this.$firebase.firestore().collection('User')
    }
  },
  methods: {
    login() {
      if(this.email == '' || this.password == '') return alert("Input blank.")
      this.loading = true
      ...
      firebase.firestore().collection('User').doc(this.email).get().then((doc) => {
          if (doc.exists) {
              ...
              {
                  if(doc.data().isAdmin == true) this.$store.commit('setUser',{email: this.email, isAdmin: true, user_name: doc.data().name, student_id: null})
                  else this.$store.commit('setUser',{email: this.email, isAdmin: false, user_name: doc.data().name, student_id: doc.data().student_id})
                  alert("Login Success!")
                  router.replace('/')
              }
