...
export default {
  ...
  watch: {
    check_password: function (val) {
      if(this.password == val) this.check_valid = true
      else this.check_valid = false
    },
    password: function (val) {
      if(val == "") this.password_valid = true
      else if(val.length > 8) this.password_valid = true
      else this.password_valid = false
    },
    email: function (val) {
      if(val == "") this.email_valid = true
      else if(val.includes('@')) this.email_valid = true
      else this.email_valid = false
    },
  },
  methods: {
    auth() {
    ...
    signUp() {
      ...
      else alert("check your input validation.")
